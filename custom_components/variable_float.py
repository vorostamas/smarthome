...
@ Date        : 09/14/2017
@ Description : Global Float Variable - We can now have global variable
                that holds Float datatype values
...
variable_float:
  some_float_number1:
    name: Some Float Number 
    icon: mdi:numeric

  some_float_number2:
    name: Some Float Number 2
    value: 12.34
    icon: mdi:numeric
...
at https://home-assistant.io/components/variable_float/
"""
import asyncio
import logging
import os
...
DEFAULT_ICON = "mdi: decimal-increase"
...
CONFIG_SCHEMA = vol.Schema({
    DOMAIN: vol.Schema({
        cv.slug: vol.Any({
            ...
            vol.Optional(ATTR_VALUE, default=DEFAULT_VALUE): float,
...
def async_set_value(hass, entity_id, value, readonly, icon):
    hass.async_add_job(hass.services.async_call(
        DOMAIN, SERVICE_SETVALUE, { ATTR_ENTITY_ID: entity_id, ATTR_VALUE: value, 
                                    ATTR_READONLY: readonly, CONF_ICON: icon }))

@asyncio.coroutine
def async_setup(hass, config):
    """Set up a variable_float."""
    ...
    for object_id, cfg in config[DOMAIN].items():
        ...
        entities.append(GlobalVariableFloat(object_id, name, 
                                            value, icon, readonly))
    ...
    def async_handler_service(service):
        """Handle a call to the variable_float services."""
...
class GlobalVariableFloat(Entity):
    """Representation of a variable_float."""

    def __init__(self, object_id, name, value, icon, readonly):
        """Initialize a variable_float."""
    ...
    @property
    def name(self):
        """Return name of the variable_float."""
        return self._name
    ...
    @property
    def state(self):
        """Return the current value of the variable_float."""
        return self._state
    ...
    def async_set_value(self, value, icon):
        try:
            ...
            _LOGGER.error("Error: '%s' is not a valid float value.", value)
